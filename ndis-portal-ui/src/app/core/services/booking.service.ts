...
export class BookingService {
  ...
  constructor(private http: HttpClient) {
    console.log(
      '[BookingService] API URL configured as:',
    ...
    return this.http.get<Booking[] | any>(this.apiUrl, { params }).pipe(
      map((response: any) => {
        ...
        if (response && Array.isArray(response.Data)) {
          console.log(
            '[BookingService] Found wrapped Data array with',
        ...
        if (Array.isArray(response)) {
          console.log(
            '[BookingService] Found direct array with',
            response.length,
            'items',
          );
          return response;
        ...
        console.log(
          '[BookingService] No valid booking data found, returning empty array',
        );
        return [];
      }),
      catchError((error) => {
        ...
        if (error.status === 401) {
          return throwError(
            () => new Error('Please log in to view your bookings.'),
          );
        }
        if (error.status === 403) {
          return throwError(
            () =>
              new Error('You do not have permission to view these bookings.'),
          );
        }
        if (error.status === 404) {
          return throwError(
            () =>
              new Error(
                'Bookings endpoint not found. Please check the backend API.',
              ),
          );
        }
        if (
          ...
          console.log(
            '[BookingService] Empty response detected - backend endpoint may not be implemented',
          );
          return of([]); // Return empty array instead of error
        }
        return throwError(
          () => new Error('Failed to load bookings. Please try again.'),
        );
      }),
    );
  }
    ...
    return this.http.post<Booking | any>(this.apiUrl, bookingData).pipe(
      ...
      map((response: any) => {
        ...
        if (
          ...
          throw new Error(
            'Invalid response from server. Booking was not created.',
          );
        }
      ...
      catchError((error) => {
        ...
        if (
          ...
          (error.error instanceof ProgressEvent || error.error === null)
        ) {
        ...
        if (error.message && error.message.includes('CORS')) {
          ...
          return throwError(
            () =>
              new Error(
                'Cannot connect to server due to CORS policy. Please check server configuration.',
              ),
          );
        }
        ...
        if (error.status === 400) {
          ...
          return throwError(
            () => new Error('Invalid booking data. Please check your inputs.'),
          );
        }
        if (error.status === 500) {
        ...
        return throwError(
          () => new Error('Failed to create booking. Please try again.'),
        );
      }),
    );
  }
      ...
      .pipe(
        map((response: any) => {
          ...
          return {
            ...
            assignedServiceId:
              data.assignedServiceId ?? data.AssignedServiceId ?? 0,
            assignedServiceName:
              data.assignedServiceName ?? data.AssignedServiceName ?? '',
        ...
        catchError((error) => {
          ...
          console.error(
            '[BookingService] Error fetching worker for booking',
            bookingId,
            error,
          );
          return of(null);
    ...
    return this.http
      ...
      .pipe(
        ...
        catchError((error) => {
          ...
          if (error.status === 403) {
            return throwError(
              () => new Error('You can only delete your own pending bookings.'),
            );
          }
          if (error.status === 404) {
            return throwError(() => new Error('Booking not found.'));
          ...
          if (error.error?.Message || error.error?.message) {
            return throwError(
              () => new Error(error.error.Message || error.error.message),
            );
          }
          return throwError(
            () => new Error('Failed to delete booking. Please try again.'),
          );
        }),
      );
  }
